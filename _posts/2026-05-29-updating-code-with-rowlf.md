...
nicely crafted `sed` one liner. But, for more complex tasks you need a
tool that can reason about the code structure a bit more. Migrating away
from [legacy Puppet
...
task. Most programming languages take source text and turn it into an
[abstract syntax
tree](https://en.wikipedia.org/wiki/Abstract_syntax_tree). Puppet uses
the same methodology, turning this code:
...
Operating on the abstract syntax tree allows you update a `$facts` var
that is accessed with `hostname` in many different source text forms:
...
edit the AST. One distinctive feature of Tree-sitter is that it does not
discard the source text after crafting the AST as do many parsers.
Tree-sitter provides functions for editing the source text and
regenerating the AST. Tree-sitter rose to fame using these capabilities
to provide syntax highlighting in
[Neovim](https://neovim.io/doc/user/treesitter/) based on an AST, rather
than using regexes as is done in
...
by `puppet-lint`. However, I added support for most of them in Rowlf:
...
Of course sometimes the diff has a bit of dog logic, but at least the
...
When using the `$facts` hash this is no longer possible as the variable
name is reserved. Unfortunately, migrating unscoped facts to structured
facts is more difficult, as you need to determine if the fact has been
shadowed by a local variable before updating. This is particularly
difficult in an ERB where you do not have the locally scoped Puppet
code. Rowlf has the ability to update unscoped facts using `-u` option,
but at present it does not do any analysis to determine if the fact has
been shadowed, so diffs would need careful manual review.
...
2.  Automatic fixing of Puppet Stdlib deprecations
3.  Puppet code formatting
